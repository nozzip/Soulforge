...
const Admin: React.FC<AdminProps> = ({
  ...
  const handleUpdateAdminNotes = async (orderId: string, notes: string) => {
    ...
    if (error) {
      ...
      setAllOrders(prev => prev.map(o => o.id === orderId ? { ...o, admin_notes: notes } : o));
    }
  };
  ...
  const handleSendWhatsApp = (order: any) => {
    ...
    const message = `*Pedido #${order.contract_number || order.id.slice(0, 8)}*\n\n` +
              ...
              <Table>
                <TableHead sx={{ bgcolor: (t) => alpha(t.palette.common.black, 0.3) }}>
                  <TableRow>
                    ...
                    <TableCell sx={{ color: 'secondary.main', fontWeight: 'bold' }}>Fecha del Pacto</TableCell>
                ...
                <TableBody>
                  {allOrders.map((order) => (
                    <React.Fragment key={order.id}>
                      <TableRow
                        ...
                        sx={{
                          cursor: 'pointer',
                          '&:hover': { bgcolor: (t) => alpha(t.palette.common.white, 0.05) },
                          bgcolor: expandedOrderId === order.id ? (t) => alpha(t.palette.secondary.main, 0.05) : 'inherit'
                        }}
                      >
                        <TableCell sx={{ color: 'grey.300', fontFamily: 'monospace' }}>
                          <Typography variant="body2" sx={{ fontWeight: 'bold' }}>#{order.contract_number || order.id.slice(0, 8)}</Typography>
                        </TableCell>
                        <TableCell sx={{ color: 'grey.400' }}>
                          <Typography variant="body2">{new Date(order.created_at).toLocaleDateString()}</Typography>
                          <Typography variant="caption" sx={{ color: 'grey.600' }}>{new Date(order.created_at).toLocaleTimeString()}</Typography>
                        </TableCell>
                        <TableCell sx={{ color: 'grey.400' }}>
                          <Typography variant="body2" sx={{ fontWeight: 'bold', color: 'common.white' }}>{order.customer_name || order.phone}</Typography>
                          <Typography variant="caption" sx={{ color: 'grey.600' }}>{order.customer_phone}</Typography>
                        </TableCell>
                        <TableCell>
                          <Stack spacing={0.5}>
                          ...
                          <Collapse in={expandedOrderId === order.id} timeout="auto" unmountOnExit>
                            ...
                            >
                              <Grid container spacing={3}>
                                {/* Customer Info */}
                                <Grid size={{ xs: 12, md: 6 }}>
                                  <Typography variant="h6" sx={{ color: 'secondary.main', fontWeight: 'bold', mb: 2, textTransform: 'uppercase', letterSpacing: 1 }}>
                                    Información del Aventurero
                                  </Typography>
                                  <Stack spacing={2}>
                                    <Box>
                                      <Typography variant="caption" sx={{ color: 'grey.500', textTransform: 'uppercase', letterSpacing: 1 }}>Número de Pacto</Typography>
                                      <Typography variant="body1" sx={{ color: 'common.white', fontWeight: 'bold', fontFamily: 'monospace' }}>
                                        #{order.contract_number || order.id.slice(0, 8)}
                                      </Typography>
                                    </Box>
                                    <Box>
                                      <Typography variant="caption" sx={{ color: 'grey.500', textTransform: 'uppercase', letterSpacing: 1 }}>Nombre</Typography>
                                      <Typography variant="body1" sx={{ color: 'common.white' }}>{order.customer_name || 'N/A'}</Typography>
                                    </Box>
                                    <Box>
                                      <Typography variant="caption" sx={{ color: 'grey.500', textTransform: 'uppercase', letterSpacing: 1 }}>Email</Typography>
                                      <Typography variant="body1" sx={{ color: 'common.white' }}>{order.customer_email || 'N/A'}</Typography>
                                    </Box>
                                    <Box>
                                      <Typography variant="caption" sx={{ color: 'grey.500', textTransform: 'uppercase', letterSpacing: 1 }}>Teléfono</Typography>
                                      <Typography variant="body1" sx={{ color: 'common.white' }}>{order.customer_phone || order.phone || 'N/A'}</Typography>
                                    </Box>
                                    <Box>
                                      <Typography variant="caption" sx={{ color: 'grey.500', textTransform: 'uppercase', letterSpacing: 1 }}>DNI</Typography>
                                      <Typography variant="body1" sx={{ color: 'common.white' }}>{order.customer_dni || 'N/A'}</Typography>
                                    </Box>
                                  </Stack>
                                </Grid>
                                ...
                                <Grid size={{ xs: 12, md: 6 }}>
                                  ...
                                  >
                                    <Typography variant="body1" sx={{ color: 'common.white', whiteSpace: 'pre-wrap' }}>
                                      {order.shipping_address || 'Sin dirección registrada'}
                                    </Typography>
                                  </Paper>

                                  <Box sx={{ mt: 3 }}>
                                    <Typography variant="caption" sx={{ color: 'grey.500', textTransform: 'uppercase', letterSpacing: 1 }}>Fecha del Pedido</Typography>
                                    <Typography variant="body1" sx={{ color: 'common.white' }}>
                                      {new Date(order.created_at).toLocaleString('es-ES', { dateStyle: 'full', timeStyle: 'short' })}
                                    </Typography>
                                  </Box>

                                  <Box sx={{ mt: 2 }}>
                                    <Typography variant="caption" sx={{ color: 'grey.500', textTransform: 'uppercase', letterSpacing: 1 }}>Total</Typography>
                                  ...
                                  <Box sx={{ mt: 2 }}>
                                    <Typography variant="caption" sx={{ color: 'grey.500', textTransform: 'uppercase', letterSpacing: 1 }}>Estado Actual</Typography>
                                    <Chip
                                      label={order.status}
                                      size="small"
                                      sx={{
                                        ...
                                        bgcolor: (theme) => {
                                          switch (order.status) {
                                            case 'Enviado': return alpha(theme.palette.success.main, 0.2);
                                            case 'En la Forja': return alpha(theme.palette.warning.main, 0.2);
                                            case 'Limpieza y Curado': return alpha(theme.palette.info.main, 0.2);
                                            case 'Recibido': return alpha(theme.palette.grey[500], 0.2);
                                            case 'Cancelado': return alpha(theme.palette.error.main, 0.2);
                                            default: return 'transparent';
                                          }
                                        },
                                        color: (theme) => {
                                          switch (order.status) {
                                            case 'Enviado': return theme.palette.success.main;
                                            case 'En la Forja': return theme.palette.warning.main;
                                            case 'Limpieza y Curado': return theme.palette.info.main;
                                            case 'Recibido': return theme.palette.grey[400];
                                  ...
                                  <Stack spacing={2}>
                                    {order.order_items?.map((item: any, i: number) => (
                                      ...
                                      >
                                        <Box
                                          component="img"
                                          src={item.image_url || 'https://via.placeholder.com/80?text=No+Img'}
                                          alt={item.name}
                                          sx={{
                                            width: 80,
                                            height: 80,
                                        ...
                                        <Typography variant="body1" sx={{ color: 'secondary.main', fontWeight: 'bold' }}>
                                          {formatCurrencyDecimal((item.unit_price || 0) * item.quantity)}
                                        </Typography>
                                      </Paper>
                                    ))}
                                  </Stack>
                                  ...
                                  <TextField
                                    ...
                                    sx={{
                                      '& .MuiOutlinedInput-root': {
                                        bgcolor: (t) => alpha(t.palette.common.black, 0.2),
                                        '& fieldset': { borderColor: (t) => alpha(t.palette.secondary.main, 0.2) },
                                  ...
                                  <Stack direction="row" spacing={2} justifyContent="flex-end">
                                    <Button
                                      ...
                                      sx={{
                                        ...
                                        '&:hover': { bgcolor: (t) => alpha(t.palette.success.main, 0.1) }
                                      }}
                                    >
                                      Enviar WhatsApp
                                    </Button>
                  ...
                  {allOrders.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={6} sx={{ textAlign: 'center', py: 6, color: 'grey.600', fontStyle: 'italic' }}>
