...
export const AdminOrders: React.FC = () => {
  ...
  const handleDeleteOrder = async (orderId: string) => {
    ...
    // Delete the order itself - cascade will handle items
    ...
    if (error) {
      ...
      showToast("Error al romper el pacto: " + error.message, "error");
    } else if (count === 0) {
      // Optimistically remove from UI to handle "ghost" orders
      setAllOrders((prev) => prev.filter((o) => o.id !== orderId));
      showToast(
        "El pedido no se encontró (quizás ya fue eliminado). Se ha quitado de la lista.",
        "warning",
      );
    } else {
      setAllOrders((prev) => prev.filter((o) => o.id !== orderId));
      showToast("Pacto roto (pedido eliminado)", "success");
    }
  };
  ...
  const handlePrintOrder = (order: Order) => {
    ...
    const itemsHtml = order.order_items
      ...
      <tr>
        ...
        <td style="padding: 8px; border-bottom: 1px solid #ddd; text-align: right;">${formatCurrencyDecimal(item.price_gp || 0)}</td>
      </tr>
    `,
      )
      .join("");
                      ...
                      <FormControl size="small" fullWidth>
                        ...
                        >
                          ...
                          <MenuItem value="En la Forja">
                            <Handyman sx={{ fontSize: 16, mr: 1 }} /> En la
                            Forja
                          </MenuItem>
